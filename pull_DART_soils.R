...
for (id in ids_to_run) {
  ...
  # extraction <- extract_TS(padpoly, padpixels, chosenCandidates, top.sims)
  ...
  # steps to recreate:
  #     inner buffer the padpolygon
  #     check if refrast and padpoly are the same CRS
  #     rasterize the padpoly using refrast
  #     crop to extent of padpoly
  #     make sure extent/crs match
  
  # or, just check to see if x and y in extraction$extractedTarget are present in padpixels$geometry ???
  ...
  # extraction$extractedTarget has x/y coordinates, but how do these connect to the soil data?
  # extractedTarget is generated by extract_TS, and = 'padvals'
  # extract_TS also requires padpoly, chosenCandidates, and top.sims; all of which are in .RData
  # padvals generated by: padvals <- terra::extract(s, padpixels, ID=T, xy=T)
  # padpixels is passed from dart_fn, this is the same padpixels saved in the .RData files
  #
  # all together:
  # extraction <- extract_TS(padpoly, padpixels, chosenCandidates, top.sims)
  # 
  
  # all the column renaming means that ID in extracted_reference is 
  # the same as the row number of chosenCandidates
  # chosenCandidates are the chosen reference pixels
  ...
  # reference_full_id is maybe the row number from the buffered, rasterized padpoly
  # extraction_id is an arbitrary row number
  # extraction_id is equivocated to extracted.pixel.id in the left_join
