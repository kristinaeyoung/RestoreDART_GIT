...
create_effects_map <- function(functional_group_of_interest) {
  ...
  # Find the maximum absolute value of point.effect to ensure symmetric color scale
  max_effect <- max(abs(trt_effects_sf$point.effect), na.rm = TRUE)
  ...
  fx_effects_map <- ggplot() +
    ...
    scale_fill_gradientn(colors = color_ramp, na.value = "transparent",
                         limits = c(-max_effect, max_effect),  # Set limits symmetric around 0
...
# Function to create treatment effect line graph
create_trt_effects_linegraph <- function(effects_table, target_id) {
  # Filter data for the years following treatment
  filtered_data <- effects_table |> filter(year.index > trt_year - 3)
  # Create the ggplot for treatment effects
  trt_effects_fig <- ggplot(data = filtered_data, aes(x = year.index, y = point.effect, group = target_id)) +
    # Line plot for each target
    geom_line(color = 'grey30', alpha = 0.5) +  
    # Add vertical line for treatment year
    geom_vline(xintercept = trt_year, color = 'black', size = 1) +  
    # Add horizontal line at 0
    geom_hline(yintercept = 0, color = 'black', linetype = 'dashed', size = 1) +  
    # Add a mean line (average of point.effect per year.index)
    stat_summary(fun = "mean", geom = "line", aes(group = 1), color = 'blue', size = 1.5) + 
    # Labels and title
    labs(x = "Year", y = "Treatment effect (per target pixel)", 
         title = "Treatment effects", 
         subtitle = paste("ID", target_id)) +
    facet_wrap(~fun_group, scales = 'free') +  # Facet by functional group
    theme_classic()
  
  ggsave(trt_effects_fig, file = paste0(dart_save_base_folder, "ID_", polyID, "_effects_line_graph.png"),
...
dart_data_folder <- "/project/aeroldc/RestoreDART/DART_running/full_dart_run_24Feb2025/" #where the Rdata outputs from DART live
dart_save_base_folder <- "/project/aeroldc/RestoreDART/DART_running/pixelwise_dart_results_25Feb2025/" # where the results will be saved

# Check if the folder is writable before proceeding with DART processing
if (file.access(dart_save_base_folder, mode = 2) == 0) {cat("You have write access to the folder.\n")} else {cat("You do NOT have write access to the folder.\n")}
# if not, consider creating a new subfolder in DART_intermediate_outputs

# grab all the Rdata files in the dart data folder
...
# print(unique_ids)
# #specify polyIDs you want to loop through:
# specific_ids = c(1)
...
## Intermediate step: figure out which IDs have already been run.
# in hthe dart_save_base_folder, get a list of all the csv files. 
# determine the IDs of those files 
# file names will be something like 	ID_1000_model_performance.csv
# remove those ids from unique IDS to not run them 

## Intermediate step: figure out which IDs have already been run.
# Get a list of all the CSV files in the dart_save_base_folder
csv_files <- list.files(dart_save_base_folder, pattern = "_model_performance\\.csv$", full.names = TRUE)

# Extract IDs from the CSV filenames
processed_ids <- gsub(".*ID_(\\d+)_model_performance\\.csv$", "\\1", csv_files)
processed_ids <- unique(as.numeric(processed_ids))

# Remove processed IDs from unique_ids to avoid re-running them
...
for (id in ids_to_run) {
  ...
  # save the shapefile of the target pixels 
  st_write(obj = padpixels, 
           paste0(dart_save_base_folder, "shapefiles/", "ID_", polyID, "_padpixels.shp"), append = FALSE)
  cat("Saved target pixel shapefile for ID:", id, "\n")
  ...
  #   geom_spatvector(data = terra::vect(model_performance_sf |> filter(!is.na(fun_group))),
  #                   aes(fill = r), color = 'transparent') +
  #   labs(title = "Model performance",fill = "r",
  #        subtitle = "Correlation between predicted + observed cover in target pixel pre-trt") +
  #   facet_wrap(~fun_group) + scale_fill_gradient(low = "deepskyblue", high = "deepskyblue4") +  # Define the color ramp
  #   theme_void()
  ...
  # create_effects_map("annual_forb_and_grass_cover")
  # create_effects_map("tree_cover")
  # 
  # cat("Saved trt effects map for functionals group for ID:", id, "\n") 
  # 
  # create_trt_effects_linegraph(combined_model_series, polyID)
  # cat("Saved treatment effects line graph for ID:", id, "\n")
  # 
  ...
  rm(trt_year, polyID, chosenCandidates, extractedReference_wide, temp_extracted_target, extracted_target, temp_extracted_reference, extracted_reference, ref_table, fun_groups, combined_model_series_list, combined_model_series, model_performance)
  gc()
}
...
### combine the pixelwise outputs into mega csv files 
dart_save_base_folder <- "/project/aeroldc/RestoreDART/DART_running/pixelwise_dart_results_25Feb2025/" # where the results will be saved
# in this folder

# identify all the csv files which include trt_effects
# ie. 	ID_1000_trt_effects.csv

# read and rbind them all 



# List all files that include "trt_effects" in the filename
...
combined_trt_effects <- bind_rows(lapply(csv_files, read_csv))
head(combined_trt_effects)
# Save the combined file
write_csv(combined_trt_effects, file.path("/project/aeroldc/RestoreDART/DART_running/trt_effects_all_RestoreDART.csv"))


post_trt_effects = combined_trt_effects |> filter(year.index >= trtYear)
write_csv(post_trt_effects, file.path("/project/aeroldc/RestoreDART/DART_running/trt_effects_post_trt_all_RestoreDART.csv"))


# # List all files that include "trt_effects" in the filename
# csv_files <- list.files(path = dart_save_base_folder, pattern = "_model_performance.*\\.csv$", full.names = TRUE)
# print(csv_files[1])
# # need to add a PolyID from the file name 


# List all files that include "trt_effects" in the filename
csv_files <- list.files(path = dart_save_base_folder, pattern = "_model_performance.*\\.csv$", full.names = TRUE)

# Extract PolyID from file names (assumes ID is the number right after "ID_" and before "_model_performance")
extract_polyid <- function(file_path) {
  str_match(basename(file_path), "ID_(\\d+)_model_performance")[,2]
}

# Read all CSV files and combine them into a single data frame, adding PolyID
combined_model_performance <- bind_rows(lapply(csv_files, function(file) {
  df <- read_csv(file)
  df$PolyID <- extract_polyid(file)
  return(df)
}))

# Convert PolyID to numeric if needed
combined_model_performance <- combined_model_performance %>%
  mutate(PolyID = as.numeric(PolyID))

head(combined_model_performance)
summary(combined_model_performance$PolyID)

# Save the combined file
write_csv(combined_model_performance, file.path("/project/aeroldc/RestoreDART/DART_running/model_performance_all_RestoreDART.csv"))
head(combined_model_performance)
# 16,086,291
# 7,255,309
